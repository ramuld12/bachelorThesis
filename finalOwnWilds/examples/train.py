...
def run_epoch(algorithm, dataset, general_logger, epoch, config, train):
    ...
    for batch in iterator:
        ...
        if train and (batch_idx+1) % config.log_every==0:
            if config.useDP:
                eps, alpha = algorithm.optimizer.privacy_engine.get_privacy_spent(config.target_delta)
                log_results(algorithm, dataset, general_logger, epoch, batch_idx, eps, alpha)

                # print eps and alpha
                general_logger.write(f"ɛ: {eps:.2f} (α: {alpha})\n")
            else:
...
def train(algorithm, datasets, general_logger, config, epoch_offset, best_val_metric):
    if config.useDP:
        ...
        assert config.virtual_batch_size % config.batch_size == 0 # VIRTUAL_BATCH_SIZE should be divisible by BATCH_SIZE
        SAMPLE_RATE = config.batch_size / len(datasets['train']['dataset'])
        N_ACCUMULATION_STEPS = int(config.virtual_batch_size/config.batch_size)
        ...
        privacy_engine.attach(algorithm.optimizer)
        privacy_engine.to(config.device)

        general_logger.write('\nPrivacy engine attached to optimizer\n')
...
def log_results(algorithm, dataset, general_logger, epoch, batch_idx, eps = 0, alpha = 0):
